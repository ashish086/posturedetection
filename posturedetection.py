...
# from tensorflow.keras.layers.concatenate import concatenate
...
def process(input_image, params, model_params):
    ...
    canvas1 = draw(input_image, all_peaks)  
...
def checkPosition(all_peaks):
    try:
        f = 0
        if (all_peaks[16]):
            a = all_peaks[16][0][0:2] 
            f = 1
        else:
            a = all_peaks[17][0][0:2]  
        ...
        print("person is not sitting in proper angle, unable to detect")
            ...
            try:
                if (all_peaks[4][0][0:2]):
                    ...
                    if (distance < (armdist + 100) and distance > (armdist - 100)): 
                        print("No Folding Hands detected")
                    else:
                        print("Folding Hands detected")
            except Exception as e:
                print("Folding Hands")
    except Exception as e:
        try:
            if(all_peaks[7][0][0:2]):
                ...
                # print(distance)
                if (distance < (armdist + 100) and distance > (armdist - 100)):
                    print("Not Folding hands detected")
                else:
                    print("Folding Hands detected")
        except Exception as e:
            print("Unable to detect posture correctly")
...
def checkKneeling(all_peaks):
    ...
    try:
        ...
        if (leftdegrees > 60  and rightdegrees > 60):  
            print("Leaning posture detected")
        elif (rightdegrees > 60):
            print("Leaning right side")
        elif (leftdegrees > 60):
            print("Leaning left side")
        else:
            print("Not Leaning at all")
...
if __name__ == '__main__':
    ...
    if vi == False:
        ...
        canvas, position = process('./sample_images/co1.jpg', params, model_params)
        showimage(canvas)
        if (position == 1):
            print("Hyperkyphosis posture detected")
        elif (position == -1):
            print("Slouching posture detected")
        else:
            print("Neutral Posture detected")
